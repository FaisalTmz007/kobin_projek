                    ...
                    <table class="table">
                        ...
                        <tbody id="table-content">
                            ...
                            <tr>
                              ...
                              <td>
                                <form action="{{ url('admin/premium/'.$transaksi->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('delete')
                                    <button type="submit" class="btn btn-dark">Refund Pembayaran</button>
                                </form>
                                @if ($transaksi->fk_supplier == 1)
                                <form action="{{ url('admin/premium/owner/'.$transaksi->fk_owner) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('put')
                                    <button type="submit" class="btn btn-warning">Konfirmasi Pembayaran</button>
                                </form>
                                @elseif($transaksi->fk_owner == 1)
                                <form action="{{ url('admin/premium/supplier/'.$transaksi->fk_supplier) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('put')
                                    <button type="submit" class="btn btn-warning">Konfirmasi Pembayaran</button>
                                </form>
                            ...
                            <div class="modal fade" id="exampleModal-{{$transaksi->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <img src="{{ asset($transaksi->bukti) }}" style="width: 100%; height: 100%; object-fit: cover"></img>
                                    </div>
